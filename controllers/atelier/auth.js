...
const atelierCont = {
  register: async (req, res) => {
    ...
    try {
      ...
      const usernameAtelier = await User.findOne({ username });
      if (usernameAtelier) {
      ...
      const refreshtoken = createRefreshToken(newAtelier._id, newAtelier.role);
      res.cookie("refreshtoken", refreshtoken, {
        httpOnly: true,
        path: "/api//atelier/refreshtoken",
        maxAge: 7 * 24 * 60 * 60 * 1000, // 7d
      });

      res.status(201).json({ accesstoken, newAtelier });
    } catch (error) {
  ...
  /*login: async (req, res) => {
    ...
    try {
      ...
      const searchedPatient = await Patient.findOne({ email });
